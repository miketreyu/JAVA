...
public class EjercicioParkingElegir {
        ...
        while (!salir) {
            ...
            switch (opcion) {
                case 1:
                    ...
                    break;
                case 2:
                    aparcarEnZonaEspecifica(parking);
                    break;
                case 3:
    ...
    // Aparca un coche en una zona específica
    public static void aparcarEnZonaEspecifica(char[][] parking) {
        Scanner sc = new Scanner(System.in);
        System.out.print("Introduce la fila donde deseas aparcar: ");
        int fila = sc.nextInt();

        if (fila >= 0 && fila < parking.length) {
            boolean aparcado = false;
            for (int j = 0; j < parking[fila].length; j++) {
                if (parking[fila][j] == 'L') {
                    parking[fila][j] = 'O';
            ...
            if (!aparcado) {
                System.out.println("No hay plazas libres en la fila especificada.");
            }
        } else {
            System.out.println("Fila fuera del rango del parking.");
        }
    }
